...
--	list_sep_last=",",
	...
	levelchanged = nil, --[[function(lvl, cfg) return cfg end]]
...
local function tprint(t, lvl, cfg)
	...
	if type(t) == "table" then
		...
		r[#r+1]="{"
		lvl=lvl+1 -- ident
		if cfg.levelchanged then cfg.levelchanged(lvl, cfg) end
		...
		for k,v in pairs(t) do
			if not skipassign or not skipassign(t,k,v) then
				...
				-- the content value
				r[#r+1]= (cfg.inline and "" or (cfg.indent):rep(lvl)) .. line .. tprint(v,lvl,cfg)..(cfg.list_sep)
			end
		end
		lvl=lvl-1 -- dedent
		if cfg.levelchanged then cfg.levelchanged(lvl, cfg) end
		r[#r+1]=(cfg.inline and "" or (cfg.indent):rep(lvl)).."}"
